...
const RESERVED = require('./reservedWord');
 ...
 const tokens2AST = tokens => {
    ...
    const parseDeclarations = () => {
        ...
        // 声明
        if (token.type === 'identifier' && ( token.value === RESERVED['const'].value || token.value === RESERVED['let'].value)) {
    ...
    const parseInitExpression = () => {
        ...
        // 函数定义
        if (token.type === 'identifier' && token.value === RESERVED['function'].value) {
            init = {
    ...
    const parseExpression = () => {
        ...
        if (token.type === 'punc' && token.value === '{') {
            ...
            while (token.type !== 'punc' && token.value !== '}') {
                if (token.type === 'identifier' && token.value === RESERVED['return'].value) {
                    let sentence = {}
